...
### 🏺 ¿Por qué el vault se llama *Naos*?

En la arquitectura de los templos egipcios, el **Naos** era el *sancta sanctorum*: la cámara más interior y sagrada, el santuario donde reposaba la imagen de la divinidad y donde se custodiaba lo más valioso del templo. No era un espacio cualquiera — era **el corazón del recinto**, al que solo unos pocos tenían acceso.

Hay dos detalles de esa arquitectura que dan nombre y sentido a este proyecto:

- **El Naos se construía primero.** Todo faraón comenzaba su construcción divina por esta cámara, y solo después se edificaba el resto del monumento a su alrededor. El conocimiento no es un anexo del trabajo: es el núcleo del que parte todo lo demás. Por eso el vault es el centro, y el agente se construye alrededor de él.

- **El Naos era la cámara del conocimiento custodiado.** Así como en el templo se guardaba la imagen sagrada, tu vault **Naos** custodia el registro de tu trabajo: cada bitácora, cada decisión, cada snippet, cada aprendizaje. Es tu santuario privado de conocimiento laboral — un espacio que solo tú habitas y que crece contigo.

El Egypcio (el escriba) trabaja en silencio para alimentar el **Naos** (la cámara del conocimiento). El escriba observa y documenta; el Naos preserva y custodia. Juntos convierten el trabajo efímero del día a día en memoria duradera.

> *"El escriba observa. El Naos preserva."*
...
Si pegas código en una nota (≥2 líneas), el agente detecta el lenguaje (SQL, Python, Bash, JS, JSON, YAML), lo extrae a un archivo `.md` aparte en la carpeta `snippets/` del vault, le pone frontmatter con metadata y lo enlaza desde la bitácora del día y desde el MOC del proyecto.
...
Los tokens se interceptan en cada llamada al LLM y se persisten agregados por día (archivo `scripts/finops_data.json` en el vault). FinOps nunca rompe el flujo principal: si algo falla, la llamada al LLM continúa normalmente.
...
### 🖼️ Prompts de análisis de imágenes configurables

Cada captura se envía al LLM con un prompt que describe qué buscar en la imagen. El Egypcio usa **dos prompts independientes** según el contexto:

| Prompt | Cuándo se usa | Qué extrae |
| --- | --- | --- |
| **Actividad** | Ventana de trabajo normal | Tarea, herramienta, categoría, URLs |
| **Reunión** | Ventana de reunión (Teams/Zoom/Meet) | Si se proyecta contenido, qué se muestra, URLs |

Ambos son **editables desde Configuraciones**, cada uno con su propia plantilla por defecto, validación del placeholder obligatorio `{titulo_ventana}` y botón de restaurar default. Esto permite ajustar el nivel de detalle del análisis (conciso, detallado, con extracción de objetos de datos) sin tocar código. La documentación incluye varios niveles predefinidos para cada prompt.

### 📚 Manuales consultables desde el chat

El vault tiene una carpeta `Manuales de Uso del Agente/` donde viven los `.md` con la documentación del propio agente. Cuando le preguntas al chat algo sobre **su funcionamiento, configuración o features** (ej. *"¿cómo configuro la temperatura?"*, *"¿qué hace El Egypcio?"*), el agente detecta la intención por keywords y **carga esos manuales al contexto** automáticamente, respondiendo con base en ellos. Si la pregunta es sobre tu trabajo (no sobre el agente), los manuales no se cargan — así no se gastan tokens innecesarios.

### ✅ Suite de QA con gate de arranque

El proyecto incluye un **paquete `tests/` con 154 tests** que cubren todos los módulos (rutas del vault, clasificación, FinOps, captura, chat, temas, detección de ventanas, cliente IA, integridad de config). Se corre con un runner único:

```bash
python tests/run_qa.py              # corre todo, reporte por módulo
python tests/run_qa.py -v           # con detalle de cada test
python tests/run_qa.py --modulo gantt   # solo un módulo
```

Lo más importante: **el QA corre automáticamente como gate antes de cada arranque** del agente. Si todos los tests pasan, el agente inicia normalmente y en silencio. Si algo falla, muestra un aviso con el detalle, escribe un log de diagnóstico (`qa_error_log.txt`) y te deja elegir entre cerrar para revisar o continuar bajo tu propio riesgo. El gate está diseñado para nunca bloquear el arranque por un error del propio runner.

Los tests están **completamente aislados**: no tocan tu `config.json` ni tu vault reales, no hacen llamadas a la API (todo mockeado, cero tokens) y corren en cualquier sistema.
...
```
📁 Agent LLM - El Egipcio/          # Carpeta del código
│
├── 📄 agente.py                    # Orquestador principal + gate de QA al arranque
├── 📄 monitor.py                   # Detección híbrida de ventana activa
├── 📄 captura.py                   # Screenshot + análisis con LLM (visión + URLs) + prompts de imagen configurables
├── 📄 bitacora.py                  # Escritura .md + wikilinks + snippets + frontmatter + archivos agregados por task
├── 📄 chat.py                      # Chat conversacional con contexto curado + carga condicional de task y manuales
...
├── 📄 utils.py                     # Rutas centralizadas del vault + carga de config + detección de Obsidian
...
│
├── 📁 tests/                       # Suite de QA pre-productiva (corre antes del arranque)
│   ├── run_qa.py                       # Runner único (python tests/run_qa.py)
│   ├── conftest.py                     # Helpers compartidos (vault temporal, config mock)
│   └── test_*.py                       # 14 módulos de test (154 tests)
│
...
│   └── LOGO_EL_EGIPCIO.png
│
└── 📁 Naos/                        # ← Vault de Obsidian (subcarpeta). Output del agente
    │
    ├── 📁 bitacoras/               # Bitácoras diarias
    │   └── bitacora_YYYY-MM-DD.md
    │
    ├── 📁 proyectos/               # MOCs + Gantt global
    │   ├── Maestro_Mallas.md           # Un .md por proyecto (MOC)
    │   ├── USS_en_Cifras.md
    │   ├── gantt_proyectos.md          # Gantt global Mermaid
    │   └── gantt_data.json             # Datos crudos del Gantt
    │
    ├── 📁 Task/                    # Archivos agregados + referencia
    │   ├── decisiones.md               # Agregados por tipo de task
    ...
    │
    ├── 📁 imagenes/                # Capturas en alta calidad
    │   └── captura_YYYY-MM-DD_HH-MM-SS.png
    │
    ├── 📁 Solicitudes/             # Solicitudes de información
    │   └── Data_Request.md
    │
    └── 📁 Manuales de Uso del Agente/   # Documentación que el chat puede leer
        ├── MANUAL_DE_USO.md
        └── nota-tecnica_prompts-clasificacion.md
```

> 💡 **Sobre la estructura:** el código y el vault están separados — el vault
> (aquí **Naos**, la cámara del conocimiento) es una subcarpeta de la carpeta
> del código. Como en los templos egipcios, el Naos es el núcleo: todo lo que
> el agente genera vive dentro de él. Todas las rutas del vault están
> centralizadas en `utils.py`, así que la organización es consistente y fácil
> de mantener.
...
```bash
git clone https://github.com/diegoevans2-arch/Agent-El-Egipcio-I.git
cd Agent-El-Egipcio-I
...
### 5. Habilitar los manuales del agente (opcional pero recomendado)

El repo incluye una carpeta **`Manuales de Uso del Agente/`** con la documentación del propio agente (el manual de uso, la nota técnica de prompts y este README). Esta carpeta alimenta la feature de **consultar al agente sobre sí mismo**: cuando le preguntas al chat *"¿cómo configuro la temperatura?"* o *"¿qué hace El Egypcio?"*, el agente lee esos manuales y responde con base en ellos.

Para activarla, **copia la carpeta `Manuales de Uso del Agente/` del repo a tu vault**:
...
├── proyectos/
├── ...
└── Manuales de Uso del Agente/     ← copia esta carpeta aquí
    ├── MANUAL_DE_USO.md
    ├── nota-tecnica_2026-06-02_prompts-clasificacion.md
    └── README.md
```

> 📌 El agente **crea automáticamente** todas las carpetas del vault al arrancar
> (bitacoras, proyectos, Task, etc.), incluida `Manuales de Uso del Agente/`.
> Pero la crea **vacía** — si no copias los `.md` adentro, el chat responderá
> que no hay manuales disponibles cuando le preguntes sobre su funcionamiento.
> Por eso este paso es necesario para esa feature específica. El resto del
> agente funciona sin él.

Puedes agregar tus propios `.md` a esa carpeta también: el agente leerá todos los que encuentre cuando le preguntes sobre su funcionamiento.
...
| `prompt_imagen_actividad` / `prompt_imagen_reunion` | Plantillas de los prompts de análisis de imágenes (placeholder `{titulo_ventana}` obligatorio) |
...
- [x] Prompts de análisis de imágenes configurables (actividad y reunión)
- [x] Manuales del agente consultables desde el chat
- [x] Vault reestructurado con rutas centralizadas (bitacoras, proyectos, Task, scripts, etc.)
- [x] Suite de QA (154 tests) con gate automático de arranque
